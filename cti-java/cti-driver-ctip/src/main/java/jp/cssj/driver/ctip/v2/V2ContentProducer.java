...
public class V2ContentProducer {
    ...
    private boolean receiveNext(boolean wait) throws IOException {
        return receiveNext(wait, io.deadline());
    }

    void nextUntil(long deadline) throws IOException {
        receiveNext(true, deadline);
    }

    private boolean receiveNext(boolean wait, long deadline) throws IOException {
        while (true) {
