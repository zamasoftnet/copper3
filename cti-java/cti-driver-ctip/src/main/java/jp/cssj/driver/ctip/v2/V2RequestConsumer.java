...
public class V2RequestConsumer {
    ...
    /** During reset, resource replies share the drain deadline and cannot reenter callbacks. */
    void missingResourceUntil(URI uri, long deadline) throws IOException {
        byte[] uriBytes = ChannelIO.toBytes(uri.toString(), this.charset);
        ByteBuffer bytes = ByteBuffer.allocate(7 + uriBytes.length);
        bytes.putInt(3 + uriBytes.length).put(V2ClientPackets.MISSING_RESOURCE)
                .putShort((short) uriBytes.length).put(uriBytes).flip();
        Packet target;
        synchronized (packetLock) {
            checkSending();
            detachData();
        ...
        io.wakeup();
        sendUntil(target, false, deadline);
    }

    boolean hasFailed() {
        synchronized (packetLock) { return sendFailure != null; }
    }
    ...
    /** Best effort abort after a response failure, with its own bounded deadline. */
    void abortAfterFailure() throws IOException {
        Packet target;
        synchronized (packetLock) {
            checkSending();
            detachData();
            ByteBuffer bytes = ByteBuffer.allocate(6);
            bytes.putInt(2).put(V2ClientPackets.ABORT).put((byte) 1);
            bytes.flip();
            target = new Packet(bytes);
            packets.add(target);
        }
        sendUntil(target, false, ChannelIO.deadline(1000));
    }
	...
	public void close() throws IOException {
        ...
        synchronized (packetLock) {
            ...
            if (sendFailure == null) {
                ...
                // The original caller already received this failure. Re-throwing
                // the same object from close breaks try-with-resources suppression.
                packets.clear();
                pos = 0;
            }
        }
        io.wakeup();
        try {
            if (target != null) { sendUntil(target, false, ChannelIO.deadline(TLSSocketChannel.CLOSE_TIMEOUT)); }
        }
        finally { io.close(); }
    }
