...
public final class TLSPolicy {
	...
	public static boolean isInsecure() {
        String value = System.getProperty(INSECURE);
        String legacy = System.getProperty(LEGACY_TRUST);
        if (legacy != null && WARNED.compareAndSet(false, true)) {
            LOG.log(Level.WARNING, LEGACY_TRUST + " is deprecated; use " + INSECURE
                    + ". Certificate verification is enabled by default; the new name takes precedence.");
        }
        return Boolean.parseBoolean(value != null ? value : legacy);
    }
}
